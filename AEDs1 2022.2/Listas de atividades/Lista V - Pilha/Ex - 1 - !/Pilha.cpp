...
int pesquisa(TipoPilha *pilha, int id){
    if(VerificaPilhaVazia(pilha)) return ;
    for (int i = pilha->topo; i >= 0; i--){
        if(pilha->topo == id) return i;
        else return -1;
    }
}

void excluiPorID(TipoPilha *pilha, int id){
    if(VerificaPilhaVazia(pilha)) return ;
    if(pesquisa(pilha, id) == -1)  cout << "Não existe na pilha";
    else{
